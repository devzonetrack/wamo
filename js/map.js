...
function deleteMapObjects(){
    console.log('Delete Maps Objects');
}
...
function formatViewGeofence(val, row) {
    ...
    html += '<tr><td></td><td style="padding:5px;"><button onclick=editGeofence(' + row.id + ');>Edit</button></td></tr>';
    html += '</table>';
    return html;
}
    ...
    downloadGeofencePoints(id, function (points) {
        ...
        if (shape !== null) {
            ...
            formGeofence.form('load', {
                ...
                mode: 'edit',
                gf_name: node.text,
                max_park: node.max_park,
                max_speed: node.max_speed,
                min_speed: node.min_speed,
                enter_alarm: node.enter_alarm == '1' ? 'on' : 'off',
                exit_alarm: node.exit_alarm == '1' ? 'on' : 'off'
            });
